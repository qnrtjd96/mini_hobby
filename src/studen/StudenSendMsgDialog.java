...
public class StudenSendMsgDialog extends JDialog {
	...
	public void sendDetailLst(String idStr, int sendMsgNum) {
   	 ...
   	 System.out.println("다이얼로그 데이터 읽어오기 > 쪽지쓰기에 아이디 받아오는지.... "+idStr+", 쪽지번호 읽히는지.... "+sendMsgNum);
		for(int i=0; i<sendLst.size(); i++) {
			ConsVO sendVo = sendLst.get(i);
       	 System.out.println("for > > 쪽지쓰기에 아이디 받아오는지.... "+idStr+", 쪽지번호 읽히는지.... "+sendMsgNum);		
			Object data[] = {
					" " + sendVo.getMsg_title(), sendVo.getMsg_detail(), sendVo.getSend(), sendVo.getGet(), sendVo.getMsg_detail()
			};
